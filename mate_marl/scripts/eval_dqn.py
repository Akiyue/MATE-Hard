"""Evaluate a trained DQN-MARL checkpoint on a scenario WITHOUT reward shaping.

Usage:
    uv run python -m mate_marl.scripts.eval_dqn \\
        --checkpoint checkpoints/dqn_4v8_v8/dqn_*.pt \\
        --scenarios MATE-4v8-9 --num-episodes 30 --seeds 0 1 2 \\
        --max-steps 5000 --device cuda \\
        --out results/dqn_v8_step_eval.csv
"""
...
def parse_args():
    ...
    p.add_argument("--checkpoint", required=True,
                   help="Path or glob pattern to .pt file")
        ...
        for seed in args.seeds:
            ...
            def env_factory(seed=seed):
                return make_marl_env_discrete(
                    ...
                    levels=args.levels,
                    reward_shaping=False,  # no shaping at eval time
                    seed=seed,
                )
            ...
            cfg = DQNConfig(
                num_envs=1, n_actions=nA,
                buffer_size=10, batch_size=4, learning_starts=10000,
            ...
            agent = DQN(env_factory, num_cameras=nC, n_actions=nA, config=cfg)
            ...
            for ep in range(args.num_episodes):
                ...
                for t in range(args.max_steps):
                    obs_t = {k: torch.as_tensor(v, device=agent.device).reshape((-1,) + v.shape[1:])
                             if v.ndim > 1 else torch.as_tensor(v, device=agent.device)
