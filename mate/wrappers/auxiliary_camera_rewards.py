...
class AuxiliaryCameraRewards(gym.Wrapper, metaclass=WrapperMeta):
    ...
    def step(
        ...
        # Some downstream MATE code still expects the old 4-tuple form
        # `(obs, reward, done, info)` — fold terminated|truncated into a
        # single `done`-equivalent for that code path, then re-emit as a
        # 5-tuple at the end.
        step_out = self.env.step(action)
        if len(step_out) == 5:
            observations, rewards, terminated, truncated, infos = step_out
            dones = terminated  # used internally below for done-style logic
        else:
            observations, rewards, dones, infos = step_out
            terminated, truncated = dones, False
